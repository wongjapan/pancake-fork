...
export function transactionErrorToUserReadableMessage(error: any, t: TranslateFunction) {
  ...
  switch (reason) {
    ...
    case 'ArborRouter: INSUFFICIENT_OUTPUT_AMOUNT':
    case 'ArborRouter: EXCESSIVE_INPUT_AMOUNT':
    case 'ArborRouter: INSUFFICIENT_A_AMOUNT':
    case 'ArborRouter: INSUFFICIENT_B_AMOUNT':
      return t(
        'This transaction will not succeed either due to price movement or fee on transfer. Try increasing your slippage tolerance.',
      )
    case 'TransferHelper: TRANSFER_FROM_FAILED':
      return t('The input token cannot be transferred. There may be an issue with the input token.')
    case 'Arbor: TRANSFER_FAILED':
