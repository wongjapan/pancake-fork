...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Arbor Squad NFT%'

  const rawText = t(`Set %Arbor Squad NFT% as Arbor Profile avatar`)
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Arbor Profile%'

  const rawText = t(`Reach %point% or more %Arbor Profile% points`, { point: pointThreshold })
