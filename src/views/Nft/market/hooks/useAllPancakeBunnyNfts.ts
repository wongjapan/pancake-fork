...
import {
  getAllArborBunniesLowestPrice,
  getAllArborBunniesRecentUpdatedAt,
  getNftsFromCollectionApi,
} from 'state/nftMarket/helpers'
import { NftToken } from 'state/nftMarket/types'
import { pancakeBunniesAddress } from '../constants'

// If collection is ArborBunnies - gets all available bunnies, otherwise - null
const useAllArborBunnyNfts = (collectionAddress: string) => {
  const [allArborBunnyNfts, setAllArborBunnyNfts] = useState<NftToken[]>(null)

  const isPBCollection = collectionAddress === pancakeBunniesAddress

  useEffect(() => {
    const fetchArborBunnies = async () => {
      // In order to not define special TS type just for ArborBunnies display we're hacking a little bit into NftToken type.
      ...
      const [lowestPrices, latestUpdates] = await Promise.all([
        getAllArborBunniesLowestPrice(bunnyIds),
        getAllArborBunniesRecentUpdatedAt(bunnyIds),
      ])
      ...
      setAllArborBunnyNfts(allBunnies)
    }
    if (isPBCollection && !allArborBunnyNfts) {
      fetchArborBunnies()
    }
  }, [isPBCollection, allArborBunnyNfts])

  return allArborBunnyNfts
}

export default useAllArborBunnyNfts
